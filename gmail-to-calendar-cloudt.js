...
const QUERY_CLOUDT_GAME = "subject:ゲーム予約内容のご案内【クラウティ】";

function main() {
  pickUpMessage(QUERY_CLOUDT, function (message) {
    parseCloudt(message);
  });
  pickUpMessage(QUERY_CLOUDT_GAME, function (message) {
    parseCloudt(message);
  });
}
...
function parseCloudt(message) {
  ...
  const datePrefix = "(レッスン|ゲーム)時間：";
  const datePrefixLesson = "レッスン時間：";
  const datePrefixGame = "ゲーム時間：";
  ...
  const parsedDate = result[0]
    .replace(datePrefixLesson, "")
    .replace(datePrefixGame, "");
